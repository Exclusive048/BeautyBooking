...
export function CatalogCard({ item, serviceQuery }: CatalogCardProps) {
  ...
  const priceText = hasServiceQuery && item.primaryService && item.primaryService.price > 0
    ? `${item.primaryService.title} ${UI_TEXT.catalog.priceAllIncluded}: ${formatPrice(item.primaryService.price)}`
    : item.minPrice && item.minPrice > 0
      ? `${UI_TEXT.catalog.priceFrom} ${formatPrice(item.minPrice)}`
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-xs text-muted-foreground">
              {showNew ? UI_TEXT.catalog.newLabel : `⭐ ${item.ratingAvg.toFixed(1)} · ${item.reviewsCount} ${UI_TEXT.catalog.reviewsLabel}`}
            </div>
          </div>
        </div>
