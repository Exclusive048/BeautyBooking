...
export function StudioServicesPanel({ studioId }: Props) {
  ...
  const endpoint = useMemo(() => `/api/studios/${studioId}/services`, [studioId]);
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border p-4 space-y-3">
        <div className="text-sm font-semibold">Добавить услугу в каталог</div>
      ...
      <div className="space-y-3">
        {items.map((item) => (
          <ServiceRow
            key={`${item.id}:${item.name}:${item.durationMin}:${item.price}:${item.isEnabled}`}
            item={item}
