...
export default async function StudioPage() {
  ...
  if (user.accountType !== "STUDIO" && user.accountType !== "STUDIO_ADMIN") redirect("/403");

  const provider = await prisma.provider.findFirst({
    where: { ownerUserId: user.id, type: "STUDIO" },
  ...
  if (!provider) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-10">
        <h1 className="text-2xl font-semibold">Кабинет студии</h1>
        <p className="mt-2 text-neutral-600">
          У вас пока нет профиля студии. Создайте его, чтобы получать записи.
        </p>
...
async function createMyStudioProviderAction() {
  ...
  // ✅ Фикс: ищем существующего провайдера ТОЛЬКО типа STUDIO
