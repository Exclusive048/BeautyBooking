...
export async function GET(req: Request) {
  try {
    ...
    if (backfill.scannedCount > 0) {
      logInfo("Schedule request archive analytics backfill executed", {
        requestId: getRequestId(req),
        route: "GET /api/studio/schedule/requests/archive/analytics/statuses",
        studioId: access.studioId,
    ...
    const statuses = await getScheduleChangeRequestArchiveStatusCounts({
      studioId: access.studioId,
      finalizedFrom: range.from,
      finalizedToExclusive: range.toExclusive,
    });
    ...
    if (appError.status >= 500) {
      logError("GET /api/studio/schedule/requests/archive/analytics/statuses failed", {
        requestId: getRequestId(req),
        route: "GET /api/studio/schedule/requests/archive/analytics/statuses",
