...
- **Features editor в admin UI** — сейчас features (boolean overrides + limit overrides + inheritance) редактируется только через legacy admin-billing.tsx или через API напрямую. Phase ADMIN-BILLING-B или отдельный коммит
